...
// mskim. From SU2-v6, add a MPI setting
void CSolver::Set_MPI_AxiAuxVar_Gradient(CGeometry *geometry, CConfig *config) {
  unsigned short iVar, iDim, iMarker, iPeriodic_Index, MarkerS, MarkerR;
  unsigned long iVertex, iPoint, nVertexS, nVertexR, nBufferS_Vector, nBufferR_Vector;
  su2double rotMatrix[3][3], *angles, theta, cosTheta, sinTheta, phi, cosPhi, sinPhi, psi, cosPsi, sinPsi,
  *Buffer_Receive_Gradient = NULL, *Buffer_Send_Gradient = NULL;

  unsigned short nAxiAuxVar = 3;

  su2double **Gradient = new su2double* [nAxiAuxVar];
  ...
  for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++) {
    ...
    if ((config->GetMarker_All_KindBC(iMarker) == SEND_RECEIVE) &&
      ...
      nBufferS_Vector = nVertexS*nAxiAuxVar*nDim;        nBufferR_Vector = nVertexR*nAxiAuxVar*nDim;
      ...
      for (iVertex = 0; iVertex < nVertexS; iVertex++) {
        ...
        for (iVar = 0; iVar < nAxiAuxVar; iVar++)
          for (iDim = 0; iDim < nDim; iDim++)
            Buffer_Send_Gradient[iDim*nAxiAuxVar*nVertexS+iVar*nVertexS+iVertex] = node[iPoint]->GetGradient(iVar, iDim);
      }
      ...
      for (iVertex = 0; iVertex < nVertexR; iVertex++) {
        for (iVar = 0; iVar < nAxiAuxVar; iVar++)
          for (iDim = 0; iDim < nDim; iDim++)
            Buffer_Receive_Gradient[iDim*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex] = Buffer_Send_Gradient[iDim*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex];
      }
      ...
      for (iVertex = 0; iVertex < nVertexR; iVertex++) {
        ...
        for (iVar = 0; iVar < nAxiAuxVar; iVar++)
          for (iDim = 0; iDim < nDim; iDim++)
            Gradient[iVar][iDim] = Buffer_Receive_Gradient[iDim*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex];

        /*--- Need to rotate the gradients for all conserved variables. ---*/
        for (iVar = 0; iVar < nAxiAuxVar; iVar++) {
          if (nDim == 2) {
            Gradient[iVar][0] = rotMatrix[0][0]*Buffer_Receive_Gradient[0*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex] + rotMatrix[0][1]*Buffer_Receive_Gradient[1*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex];
            Gradient[iVar][1] = rotMatrix[1][0]*Buffer_Receive_Gradient[0*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex] + rotMatrix[1][1]*Buffer_Receive_Gradient[1*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex];
          }
          else {
            Gradient[iVar][0] = rotMatrix[0][0]*Buffer_Receive_Gradient[0*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex] + rotMatrix[0][1]*Buffer_Receive_Gradient[1*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex] + rotMatrix[0][2]*Buffer_Receive_Gradient[2*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex];
            Gradient[iVar][1] = rotMatrix[1][0]*Buffer_Receive_Gradient[0*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex] + rotMatrix[1][1]*Buffer_Receive_Gradient[1*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex] + rotMatrix[1][2]*Buffer_Receive_Gradient[2*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex];
            Gradient[iVar][2] = rotMatrix[2][0]*Buffer_Receive_Gradient[0*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex] + rotMatrix[2][1]*Buffer_Receive_Gradient[1*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex] + rotMatrix[2][2]*Buffer_Receive_Gradient[2*nAxiAuxVar*nVertexR+iVar*nVertexR+iVertex];
          }
        }
...
void CSolver::SetAxiAuxVar_Gradient_LS(CGeometry *geometry, CConfig *config) {
  ...
  unsigned short nDim = geometry->GetnDim();
  unsigned long iPoint, jPoint;
  su2double *Coord_i, *Coord_j, *AxiAuxVar_i, *AxiAuxVar_j, weight, r11, r12, r13, r22, r23, r23_a,
  r23_b, r33, z11, z12, z13, z22, z23, z33, detR2, product;
  bool singular = false;
  unsigned short nAxiAuxVar = 3;

  su2double **AxiCvector = new su2double* [nAxiAuxVar];
  for (iVar = 0; iVar < nAxiAuxVar; iVar++) {
    AxiCvector[iVar] = new su2double [nDim];
  }
  ...
  for (iPoint = 0; iPoint < geometry->GetnPoint(); iPoint++) {
    ...
    /*--- Inizialization of variables ---*/
	// AxiCvector[iVar][iDim] == [3][2]
	for (iVar = 0; iVar < nAxiAuxVar; iVar++) {	
  	  for (iDim = 0; iDim < nDim; iDim++)
        AxiCvector[iVar][iDim] = 0.0;
	}
    ...
    for (iNeigh = 0; iNeigh < geometry->node[iPoint]->GetnPoint(); iNeigh++) {
      ...
      if (fabs(weight) > EPS) {
        ...
        for (iVar = 0; iVar < nAxiAuxVar; iVar++) {
          for (iDim = 0; iDim < nDim; iDim++)
            AxiCvector[iVar][iDim] += (Coord_j[iDim]-Coord_i[iDim])
								     *(AxiAuxVar_j[iVar]-AxiAuxVar_i[iVar])/(weight);
        }
	  }
    ...
    for (iVar = 0; iVar < nAxiAuxVar; iVar++) {
      for (iDim = 0; iDim < nDim; iDim++) {
        product = 0.0;
        for (jDim = 0; jDim < nDim; jDim++)
          product += Smatrix[iDim][jDim]*AxiCvector[iVar][jDim];
        if (geometry->node[iPoint]->GetDomain())
          node[iPoint]->SetAxiAuxVarGradient(iVar, iDim, product);
      }
	}
  }
