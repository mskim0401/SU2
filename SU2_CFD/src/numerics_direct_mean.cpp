...
void CSourceAxisymmetric_Flow::ComputeResidual(su2double *val_residual, su2double **val_Jacobian_i, CConfig *config) {
...
// V_i = primitive variables : T, u, v, w, P, rho, h, ...
    ...
    if (implicit) {
      ...
      val_Jacobian_i[0][2] = 1.0;
      val_Jacobian_i[0][3] = 0.0;
      
      val_Jacobian_i[1][0] = -U_i[1]*U_i[2]/(U_i[0]*U_i[0]);
      val_Jacobian_i[1][1] = U_i[2]/U_i[0];
      val_Jacobian_i[1][2] = U_i[1]/U_i[0];
      val_Jacobian_i[1][3] = 0.0;
      
      val_Jacobian_i[2][0] = -U_i[2]*U_i[2]/(U_i[0]*U_i[0]);
      val_Jacobian_i[2][1] = 0.0;
      val_Jacobian_i[2][2] = 2*U_i[2]/U_i[0];
      val_Jacobian_i[2][3] = 0.0;
      
      val_Jacobian_i[3][0] = -Gamma*U_i[2]*U_i[3]/(U_i[0]*U_i[0]) + (Gamma-1)*U_i[2]*(U_i[1]*U_i[1]+U_i[2]*U_i[2])/(U_i[0]*U_i[0]*U_i[0]);
      val_Jacobian_i[3][1] = -(Gamma-1)*U_i[2]*U_i[1]/(U_i[0]*U_i[0]);
      val_Jacobian_i[3][2] = Gamma*U_i[3]/U_i[0] - 1/2*(Gamma-1)*( (U_i[1]*U_i[1]+U_i[2]*U_i[2])/(U_i[0]*U_i[0]) + 2*U_i[2]*U_i[2]/(U_i[0]*U_i[0]) );
      val_Jacobian_i[3][3] = Gamma*U_i[2]/U_i[0];
      
      for (iVar=0; iVar < nVar; iVar++)
        for (jVar=0; jVar < nVar; jVar++)
          val_Jacobian_i[iVar][jVar] *= yinv*Volume;
    }
...
void CSourceAxisymmetric_Flow::ResidualDiffusion(su2double *val_residual, CConfig *config){
...
// origin... TWO3*(2*PrimVar_Grad_i[1][1] --> [2][1] ...
//    val_residual[3] -= Volume*(yinv*(total_viscosity_i*(u*(PrimVar_Grad_i[2][0]+PrimVar_Grad_i[1][1])
...
// mskim
void CSourceGeneralAxisymmetric_Flow::ComputeResidual(su2double *val_residual, su2double **val_Jacobian_i, CConfig *config) {
  unsigned short iVar, jVar;
  ...
  if (Coord_i[1] > EPS) {
    ...
    if (implicit) {
      ...
      val_Jacobian_i[1][0] = -Velocity1_i*Velocity2_i;
      val_Jacobian_i[1][1] = Velocity2_i;
      val_Jacobian_i[1][2] = Velocity1_i;
      val_Jacobian_i[1][3] = 0.0;

      val_Jacobian_i[2][0] = -Velocity2_i*Velocity2_i;
      val_Jacobian_i[2][1] = 0.0;
      val_Jacobian_i[2][2] = 2*Velocity2_i;
      val_Jacobian_i[2][3] = 0.0;

      val_Jacobian_i[3][0] = Velocity2_i*(dPdrho_e_i + dPde_rho_i/Density_i*(Velocity1_i*Velocity1_i
                                                                       + Velocity2_i*Velocity2_i
                                                                       - Energy_i) - Enthalpy_i);
      val_Jacobian_i[3][1] = -Velocity1_i*Velocity2_i/Density_i *dPde_rho_i;
      val_Jacobian_i[3][2] = Enthalpy_i - Velocity2_i*Velocity2_i/Density_i *dPde_rho_i;
      val_Jacobian_i[3][3] = Velocity2_i + Velocity2_i/Density_i *dPde_rho_i;
